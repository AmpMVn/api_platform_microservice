...
#[AsCommand(
    name: 'do:sync:rs:objects:categories',
...
class CategoriesCommand extends Command
{
    ...
    {
        $output->writeln("\n<info>Rentsoft => Objects, Categories</>");
        ...
        foreach ($clients as $client) {
            ...
            ### LEVEL ONE ###
            $postFields = [
                "sql" => "SELECT * FROM config_car_category WHERE client_id = ".$clientRentsoftId." AND level = 0 AND name != '' ORDER BY level ASC",
            ];
            ...
            {
                $syncedCategory = $this->em->getRepository(UpdatedRentsoft::class)->findOneBy([ 'remoteAction' => 'settings-category-object', 'remoteId' => $remoteCategory->id ]);
                ...
                if (!$syncedCategory) {
                    ...
                    $syncRs->setRemoteAction('settings-category-object');
            ...
            ### LEVEL TWO ###
            $postFields = [
                "sql" => "SELECT * FROM config_car_category WHERE client_id = ".$clientRentsoftId." AND level = 1 AND name != '' ORDER BY level ASC",
            ];
            ...
            {
                $syncedCategory = $this->em->getRepository(UpdatedRentsoft::class)->findOneBy([ 'remoteAction' => 'settings-category-object', 'remoteId' => $remoteCategory->id ]);
                $syncedParentCategory = $this->em->getRepository(UpdatedRentsoft::class)->findOneBy([ 'remoteAction' => 'settings-category-object', 'remoteId' => $remoteCategory->parent_id ]);
            ...
            ### LEVEL THREE ###
            $postFields = [
                "sql" => "SELECT * FROM config_car_category WHERE client_id = ".$clientRentsoftId." AND level = 2 AND name != '' ORDER BY level ASC",
            ];
            ...
            ### LEVEL FOUR ###
            $postFields = [
                "sql" => "SELECT * FROM config_car_category WHERE client_id = ".$clientRentsoftId." AND level = 3 AND name != '' ORDER BY level ASC",
            ];
            ...
            ### LEVEL FIVE ###
            $postFields = [
                "sql" => "SELECT * FROM config_car_category WHERE client_id = ".$clientRentsoftId." AND level = 4 AND name != '' ORDER BY level ASC",
            ];
        ...
        $output->writeln("<info>Rentsoft => Objects, Categories (" . $x . " created, " . $y . " updated, ".$z." skipped [because no parent found])</>");
